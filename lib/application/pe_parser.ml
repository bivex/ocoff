...
let parse_pe_file buf =
  ...
  if Bytes.get_uint8 mz 0 <> 0x4d || Bytes.get_uint8 mz 1 <> 0x5a then
    ...
    if Bytes.to_string sig_bytes <> pe_signature then
      ...
      let* symbol_table =
        Pe_symbol_parser.parse_symbol_table r
          coff_hdr.pointer_to_symbol_table
          sym_count
      in
      let* string_table =
        Pe_symbol_parser.parse_string_table r
          coff_hdr.pointer_to_symbol_table
          sym_count
      in
      Ok (Pe_file.make
...
let parse_archive buf =
  Archive_parser.parse_archive ~parse_coff_object buf
...
let parse_any buf =
  ...
  then
    Result.map (fun f -> `Pe f) (parse_pe_file buf)
          ...
          Bytes.sub_string buf 0 8 = Archive_parser.archive_signature
  then
